...
/* 
	...
	let params = {
		...
		pstrOrderDateFrom: '2024-08-30',
		pstrOrderDateTo:'2024-09-05',
	};
 ...
 fs.readFile("datam240901.xml", "utf8", function (err, data) {
				...
				datafinal.forEach((objeto) => {
					...
					if (Array.isArray(GroupsArray)) {
						const GroupsList = GroupsArray.map((item) => {
							...
							const referenciaObjeto = dataX.find(
			...
			const datasuperfinal = datafinal.map((item) => {
				...
				const filteredData = datasuperfinal.filter((item) => {
					if (item && item.HIS && item.TestID) {
						...
						//res.json({ resultados: testIDs})
                  // fs.writeFileSync('test.json',testIDs)
						return !dat.some(
							(it) =>
								it.NUMERO_DE_ORDEN.toString() === item.HIS &&
								 testIDs.some(
									(testObj) => testObj.TestID === it.NUMERO_DE_ITEM.trim()
								) 
						); 
					}

					//console.log(`${it.NUMERO_DE_ORDEN}-- ${it.NUMERO_DE_ORDEN.toString() === item.HIS}--${Number(item.HIS)} `),

					//   if (item && item.HIS && item.GroupID) {
					// 	const testIDs = Array.isArray(item.GroupID)
					// 		? item.GroupID
					// 		: [item.GroupID];

					// 	return !dat.some(
					// 		(it) =>
					// 			it.NUMERO_DE_ORDEN === Number(item.HIS) &&
					// 			testIDs.some(
					// 				(testObj) => testObj.GroupID === it.NUMERO_DE_ITEM.trim()
					// 			)
					// 	);
					// }
