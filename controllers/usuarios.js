...
const usuariosPost = async (req, res) => {
  try {
    ...
    const user = new Usuario({
      ...
      codigo_doctor: doctor.ValueCode,
      usuario,
      password,
      rol,
    });
...
const usuariosUpdate = async (req, res) => {
  ...
  const existeUser = await Usuario.findByPk(id);
  if (!existeUser) {
    return res.status(400).json({ ok: true, msg: "no existe usuario" });
  }

  const { password, doctor, codigo_doctor, ...resto } = req.body;

  const existeUserDB = await Usuario.findOne({
    where: { usuario: resto.usuario },
  });
  // console.log('existeUserDB', existeUserDB)
